...
def worker(results_queue, host="127.0.0.1", port="5559", insert_limit = 15000):
    """
    ...
    context = zmq.Context()
    socket_zmq_pull = context.socket(zmq.PULL)
    socket_zmq_pull.connect(f'tcp://{host}:{port}')
    socket_zmq_pull.setsockopt(zmq.SNDHWM, 100000)
    socket_zmq_pull.setsockopt(zmq.RCVHWM, 100000)
    ...
    while True:
        message = socket_zmq_pull.recv_string()
...
if __name__ == "__main__":
    ...
    for procnum in range(num_workers):
        workers[procnum] = Process(name=f"Puller({procnum + 1})",target=worker, args=(results_queue,))
